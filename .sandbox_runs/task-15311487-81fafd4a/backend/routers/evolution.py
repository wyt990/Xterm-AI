...
async def _compile_task_from_intent(intent: str) -> Dict[str, Any]:
    ...
    return _normalize_compiled_task(parsed, fallback_title="AI自动编译任务")


def _run_preflight(task_compiled: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return runtime.execute_task(fake_task)


async def _repair_task_from_error(intent: str, compiled_task: Dict[str, Any], preflight_result: Dict[str, Any]) -> Dict[str, Any]:
    ai = _build_intent_ai_handler()
    ...
    return _normalize_compiled_task(repaired, fallback_title=compiled_task.get("title") or "AI自动编译任务")
...
async def execute_intent(data: EvolutionIntentExecuteModel, current_user: AuthUser):
    ...
    compiled = await _compile_task_from_intent(intent)
    ...
    preflight_result = _run_preflight(compiled)
    debug_round = 0
    while (not preflight_result.get("ok")) and debug_round < int(data.max_debug_rounds):
        debug_round += 1
        compiled = await _repair_task_from_error(intent, compiled, preflight_result)
        format_err = _validate_payload_format(compiled)
        if format_err:
