    ...
    if (msg.type === 'status_card') {
        ...
        return `
            <div class="ai-rich-card ai-status-card">
                ...
                <div class="ai-rich-card-body">${escapeHtmlWithLineBreaks(msg.detail || '')}</div>
                ${renderInlineActions(msg)}
            </div>
        `;
    }
...
async function executeAutoIntent(text) {
    ...
    try {
        ...
        const trace = Array.isArray(ret?.trace) ? ret.trace : [];
        if (trace.length) {
            const lines = trace.map((x) => {
                const t = String(x?.time || '');
                const stage = String(x?.stage || '');
                const msg = String(x?.message || '');
                const detail = x?.detail ? ` | ${String(x.detail)}` : '';
                return `${t} - [${stage}] ${msg}${detail}`;
            });
            appendMessage('assistant', 'status_card', '', {
                title: '自动执行过程日志',
                status: String(ret?.status || 'running'),
                detail: lines.join('\n'),
            });
        }
        ...
        if (String(ret?.status || '') === 'failed') {
            notify(`自动编译执行失败：${ret?.error || '未知错误'}`, 'error');
        }
        saveState();
        renderMessages();
        renderSessionList();
...
async function createTaskFromAction(action) {
    ...
    const normalizeRuntimePayload = (rawPayload) => {
        ...
        // 兼容模型将 commands/verify_commands/allow_write_paths 直接放在 create_task.payload 顶层
        const direct = {};
    ...
    const hasEscapedNewlineInPython = (cmd) =>
        cmd.toLowerCase().startsWith('python -c ') && (cmd.includes('\\n') || cmd.includes('\\r'));
    const badCommand = commands.find(hasEscapedNewlineInPython)
        || (Array.isArray(taskData.payload?.verify_commands)
            ...
            : null);
    if (badCommand) {
        notify('创建任务失败：python -c 命令不允许包含 \\n/\\r 转义，请让 AI 输出单行分号写法', 'error');
        return null;
    }
    if (!commands.length) {
    ...
    let ret = null;
    let usedTitle = taskData.title;
    for (let i = 0; i < 3; i += 1) {
        const tryTitle = buildRetryTitle(taskData.title, i);
        try {
            ret = await api.createEvolutionTask({ ...taskData, title: tryTitle });
            usedTitle = tryTitle;
            break;
        } catch (err) {
