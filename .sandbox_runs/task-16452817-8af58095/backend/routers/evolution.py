...
def _trace_add(trace: list[dict[str, Any]], stage: str, message: str, detail: Any = None):
    ...
    if detail is not None:
        try:
            item["detail"] = detail if isinstance(detail, (str, int, float, bool)) else json.dumps(detail, ensure_ascii=False)[:1200]
        except Exception:
...
async def _compile_task_from_intent(intent: str, trace: list[dict[str, Any]]) -> Dict[str, Any]:
    ...
    _trace_add(trace, "compile.raw", "AI 编译响应已返回", {"raw_preview": str(raw)[:300]})
    candidate = _extract_json_candidate(raw)
    try:
        parsed = json.loads(candidate)
    except Exception as e:
        _trace_add(trace, "compile.parse_error", "编译 JSON 解析失败", {"error": str(e), "candidate_preview": str(candidate)[:300]})
    ...
    if trace is not None:
        _trace_add(
            ...
            "预演结束",
            {
                ...
                "stages": [x.get("stage") for x in (result.get("logs") or [])],
            },
        )
    return result
...
async def _repair_task_from_error(
    ...
    _trace_add(trace, "repair.raw", "AI 修复响应已返回", {"raw_preview": str(raw)[:300]})
    candidate = _extract_json_candidate(raw)
    try:
        repaired = json.loads(candidate)
    except Exception as e:
        _trace_add(trace, "repair.parse_error", "修复 JSON 解析失败", {"error": str(e), "candidate_preview": str(candidate)[:300]})
