...
function taskRowActions(task) {
    const actions = [];
    actions.push(`<button class="btn btn-sm btn-secondary" onclick="cloneEvolutionTask(${task.id})">复制</button>`);
...
export async function cloneEvolutionTask(taskId) {
    try {
        const task = evolutionTaskCache.find((x) => x.id === taskId) || await api.getEvolutionTask(taskId);
        const defaultTitle = `${task?.title || '任务'}-副本`;
        const title = (prompt('请输入复制后的任务名称：', defaultTitle) || '').trim();
        if (!title) {
            notify('复制已取消：任务名称不能为空', 'warning');
            return;
        }
        const ret = await api.cloneEvolutionTask(taskId, { title });
        notify(`任务已复制为 #${ret.task_id}`, 'success');
        await loadEvolutionTasks();
    } catch (e) {
...
export function initEvolutionCenterModule() {
    ...
    globalThis.cloneEvolutionTask = cloneEvolutionTask;
    ...
    const doResetEvolutionDemo = async (full) => {
        const tip = full
            ? '确认全量重置到干净状态？\n将清空任务、运行日志、经验库、失败报告、迁移审计。'
            : '确认普通重置？\n仅清空任务与运行日志，保留经验库/失败报告/迁移审计。';
        const ok = confirm(tip);
        if (!ok) return;
        const ret = await api.resetEvolutionDemo({ full });
        notify(ret?.message || '重置完成', 'success');
        evolutionTaskCache = [];
        await loadEvolutionTasks();
    };
    globalThis.resetEvolutionDemoNormal = async () => {
        try {
            await doResetEvolutionDemo(false);
        } catch (e) {
            notify(`普通重置失败: ${e.message}`, 'error');
        }
    };
    globalThis.resetEvolutionDemo = async () => {
        try {
            await doResetEvolutionDemo(true);
        } catch (e) {
            notify(`全量重置失败: ${e.message}`, 'error');
        }
    };
