...
async function createTaskFromAction(action) {
    ...
    const normalizeCommands = (list) => {
        ...
        return list
            ...
            .filter(Boolean)
            .map((cmd) => {
                // 某些模型会在 python -c 中输出字面量 \n，Windows shell 下会导致 SyntaxError
                if (!cmd.toLowerCase().startsWith('python -c ')) return cmd;
                // 若命令本身通过 exec("...\\n...") 承载多行 Python，不能盲目替换换行转义
                if (cmd.includes('exec(') || cmd.includes('exec("') || cmd.includes("exec('")) return cmd;
