...
function renderMessage(msg) {
    ...
    if (msg.type === 'file') {
        ...
        item.innerHTML = `${selectBox}<div class="message-text-content">${escapeHtml(msg.content || '')}</div>${renderInlineActions(msg)}${renderMessageActions(msg)}`;
    }
...
async function executeInlineAction(action, msg) {
    ...
    if (type === 'create_task') return createTaskFromAction(action);
