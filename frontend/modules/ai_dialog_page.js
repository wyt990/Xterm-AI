...
function parseQuotedValue(raw, keyPattern) {
    ...
    if (!m?.[1]) return '';
    return String(m[1]).trim().replaceAll(/^["'`]+|["'`]+$/g, '');
}
...
function parseNumberedCommands(raw, sectionKeyPattern) {
    ...
    for (const line of lines) {
        ...
        if (!m?.[2]) continue;
        const cmd = String(m[2]).trim().replaceAll(/^["'`]+|["'`]+$/g, '');
...
function extractCreateTaskPayloadFromRaw(raw, fallbackTitle) {
    const title = parseQuotedValue(raw, String.raw`title|task\.title`) || String(fallbackTitle || '').trim() || 'AI动作创建任务';
    const taskType = parseQuotedValue(raw, String.raw`task_type|task\.type`) || 'fix';
    const scope = parseQuotedValue(raw, 'scope') || 'backend';
    const riskLevel = parseQuotedValue(raw, String.raw`risk_level|risk\.level`) || 'medium';
    const acceptance = parseQuotedValue(raw, String.raw`acceptance_criteria|acceptance\.criteria`);
    const rollback = parseQuotedValue(raw, String.raw`rollback_plan|rollback\.plan`);
    const allowWritePaths = parseArrayJsonValue(raw, String.raw`payload_json\.allow_write_paths|allow_write_paths`);
    const commands = parseNumberedCommands(raw, String.raw`payload_json\.commands|commands`);
    const verifyCommands = parseNumberedCommands(raw, String.raw`payload_json\.verify_commands|verify_commands`);
...
function shouldUseAutoIntent(text) {
    const t = String(text || '').trim();
    if (!t) return false;
    if (t.startsWith('/')) return false;
    // 口语化改动诉求（默认走自动编译执行）
    return /(修改|改成|改为|替换|改回|恢复|文案|标题|页面)/.test(t);
}

async function executeAutoIntent(text) {
    setProcessing(true);
    const placeholder = appendMessage('assistant', 'text', '正在理解需求并自动编译任务，请稍候...', { streaming: true });
    saveState();
    renderMessages();
    try {
        const ret = await api.executeEvolutionIntent({
            intent: String(text || '').trim(),
            auto_run: true,
            max_debug_rounds: 2,
        });
        if (placeholder?.id) removeMessageById(placeholder.id);
        const messages = normalizeStructuredMessages({ messages: Array.isArray(ret?.messages) ? ret.messages : [] });
        if (messages.length) {
            messages.forEach((msg) => appendMessage(msg.role, msg.type, msg.content || '', msg));
        } else {
            appendMessage('assistant', 'text', '自动编译已完成，但未返回结构化卡片。请到任务中心查看最新任务。');
        }
        saveState();
        renderMessages();
        renderSessionList();
        ...
        if (placeholder?.id) removeMessageById(placeholder.id);
        const msg = `自动编译执行失败：${err?.message || err}`;
        appendMessage('assistant', 'text', msg);
        notify(msg, 'error');
        saveState();
        renderMessages();
        renderSessionList();
...
function openSocket(messages) {
    ...
    let full = '';
    const placeholder = appendMessage('assistant', 'text', 'AI 正在分析并生成结构化结果，请稍候...', { streaming: true });
    saveState();
    renderMessages();
...
async function sendMessage(textOverride = '') {
    ...
    const rawText = String(textOverride || input.value || '');
    const trimmed = rawText.trim();
    if (!trimmed) return;
    input.value = '';

    const rich = parseRichInput(trimmed);
    if (rich) {
        if (rich.type === 'command_query') {
            showCommandQueryResult(trimmed, rich.payload?.keyword || '');
            saveState();
            renderMessages();
            renderSessionList();
            return;
    ...
    // 保留用户原始输入（仅空消息被拦截），避免模板/JSON 在发送前被改写
    appendMessage('user', 'text', rawText);
    saveState();
    renderMessages();
    renderSessionList();
    if (shouldUseAutoIntent(trimmed)) {
        await executeAutoIntent(rawText);
        return;
    }
...
async function createTaskFromAction(action) {
    ...
    const normalizeRuntimePayload = (rawPayload) => {
        if (!rawPayload || typeof rawPayload !== 'object') return {};
        if (rawPayload.payload && typeof rawPayload.payload === 'object') return rawPayload.payload;
        if (rawPayload.payload_json && typeof rawPayload.payload_json === 'object') return rawPayload.payload_json;
        const direct = {};
        if (Array.isArray(rawPayload.commands)) direct.commands = rawPayload.commands;
        if (Array.isArray(rawPayload.verify_commands)) direct.verify_commands = rawPayload.verify_commands;
        if (Array.isArray(rawPayload.allow_write_paths)) direct.allow_write_paths = rawPayload.allow_write_paths;
        return direct;
    };
    const normalizeCommands = (list) => (Array.isArray(list) ? list.map((cmd) => String(cmd || '').trim()).filter(Boolean) : []);
    const runtimePayload = normalizeRuntimePayload(payload);
    runtimePayload.commands = normalizeCommands(runtimePayload.commands);
    runtimePayload.verify_commands = normalizeCommands(runtimePayload.verify_commands);
    const taskData = {
        ...
        acceptance_criteria: String(payload.acceptance_criteria || '').trim(),
        rollback_plan: String(payload.rollback_plan || '').trim(),
        max_retries: Number(payload.max_retries || 100) || 100,
        source: 'ai',
        payload: runtimePayload,
    };
    const commands = Array.isArray(taskData.payload?.commands) ? taskData.payload.commands.filter(Boolean) : [];
    const escapedN = String.raw`\n`;
    const escapedR = String.raw`\r`;
    const hasEscapedNewlineInPython = (cmd) =>
        cmd.toLowerCase().startsWith('python -c ') && (cmd.includes(escapedN) || cmd.includes(escapedR));
    const badCommand = commands.find(hasEscapedNewlineInPython)
        || (Array.isArray(taskData.payload?.verify_commands)
            ? taskData.payload.verify_commands.find(hasEscapedNewlineInPython)
            : null);
    if (badCommand) {
        notify(`创建任务失败：python -c 命令不允许包含 ${escapedN}/${escapedR} 转义，请让 AI 输出单行分号写法`, 'error');
        return null;
    }
    if (!commands.length) {
        notify('创建任务失败：payload.commands 为空，请让 AI 返回可执行步骤', 'error');
        return null;
    }
    const buildRetryTitle = (baseTitle, attempt) => {
        const safeBase = String(baseTitle || 'AI动作创建任务').trim() || 'AI动作创建任务';
        if (attempt <= 0) return safeBase;
        const now = new Date();
        const stamp = `${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}${String(now.getSeconds()).padStart(2, '0')}`;
        return `${safeBase}-副本${attempt}-${stamp}`;
    };
    const createEvolutionTaskWithRetry = async (baseData) => {
        let ret = null;
        let usedTitle = baseData.title;
        for (let i = 0; i < 3; i += 1) {
            const tryTitle = buildRetryTitle(baseData.title, i);
            try {
                ret = await api.createEvolutionTask({ ...baseData, title: tryTitle });
                usedTitle = tryTitle;
                break;
            } catch (err) {
                const msg = String(err?.message || err || '');
                const isDupTitle = msg.includes('Task title already exists');
                if (!isDupTitle || i >= 2) throw err;
            }
        }
        return { ret, usedTitle };
    };
    const { ret, usedTitle } = await createEvolutionTaskWithRetry(taskData);
    if (!ret) return null;
    ...
    const titleTip = usedTitle === taskData.title ? '' : `（标题已自动改为：${usedTitle}）`;
    notify(`已创建任务 #${taskId}${titleTip}`, 'success');
    ...
    list.addEventListener('click', async (event) => {
        ...
        if (actionBtn instanceof HTMLElement) {
            ...
            try {
                ...
                notify(`操作失败: ${err?.message || err}`, 'error');
                console.error('AI 卡片动作执行失败:', err);
            }
            return;
        }
